...
import { Trash2, Eye, EyeOff, Layers, Settings, Image as ImageIcon, Music, Sliders, Upload, ImagePlus, Type as TypeIcon, X, Mountain, TreePine, Zap, Snowflake, Circle, Square, Star, Lock, Frame, BoxSelect, Box, ChevronLeft, ChevronRight } from 'lucide-react';
...
const ControlPanel: React.FC<Props> = ({
  ...
  return (
    <div className={`absolute top-0 left-0 h-full transition-all duration-300 bg-slate-900/90 backdrop-blur-md border-r border-slate-700 flex flex-col text-slate-100 z-10 shadow-2xl ${isCollapsed ? 'w-12' : 'w-80'}`}>
      
      {/* Toggle Button */}
      <button 
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute -right-3 top-6 bg-slate-700 hover:bg-cyan-500 text-white p-0.5 rounded-full shadow-lg border border-slate-600 z-50 transition-colors"
      >
      ...
      {isCollapsed ? (
        ...
        <div className="flex flex-col items-center py-4 gap-6">
            <h1 className="text-xs font-bold text-transparent bg-clip-text bg-gradient-to-b from-cyan-400 to-purple-500 rotate-180" style={{ writingMode: 'vertical-rl' }}>
              NEBULA
            </h1>
            <button onClick={() => { setActiveTab('layers'); setIsCollapsed(false); }} className={`p-2 rounded-lg transition-colors ${activeTab === 'layers' ? 'bg-slate-800 text-cyan-400' : 'text-slate-500 hover:text-white'}`}>
                <Layers size={20} />
            </button>
            <button onClick={() => { setActiveTab('scene'); setIsCollapsed(false); }} className={`p-2 rounded-lg transition-colors ${activeTab === 'scene' ? 'bg-slate-800 text-cyan-400' : 'text-slate-500 hover:text-white'}`}>
                <Settings size={20} />
            </button>
        </div>
      ) : (
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {SCENE_PRESETS.map(preset => (
                                    ...
                                    >
                                        ...
                                        {preset.id === 'christmas_tree' && <TreePine size={20} className="text-green-400" />}
